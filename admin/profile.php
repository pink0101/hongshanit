...
<script>
    $('#avatar').on('change',function () {
        ...
        var $this=$(this);
        var files=$this.prop('files');//返回值
        if (!files.length) return ;
        //拿到我们要上传的文件
        var file=files[0];

        //FormData 是html5当中新增的一个成员,专门配合ajax操作  用于在客户端与服务端之间传递二进制数据
        var data=new FormData();
        data.append('avatar',file);//append() 方法在被选元素的结尾插入指定内容。

        var xhr=new XMLHttpRequest();
        xhr.open('POST','../admin/api/upload.php');
        xhr.send(data);//借助于 form data 传递二进制

        xhr.onload=function () {
            // console.log(this.responseText);//获得字符串形式的响应数据。
            $this.siblings('img').attr('src',this.responseText);//siblings() 方法返回被选元素的所有同级元素。
            $this.siblings('input').val(this.responseText);//val  设置value属性值
        }
    })
